...
int ft5x0x_write_reg(struct i2c_client *client, u8 addr, const u8 val)
{
	u8 buf[2] = {0};
...
int ft5x0x_read_reg(struct i2c_client *client, u8 addr, u8 *val)
{
...
static u32 gpio_reset=0;
void ft5x0x_chip_reset(void)
{
...
EXPORT_SYMBOL(ft5x0x_chip_reset);
...
int TP_charger_change_state(int state)
{
   int ret =0;
   if(cur_state==state){
	    return 0;
        ...
        msleep(100);
        if(state==1){
            ret=ft5x0x_write_reg(this_client,0x8b,0x01);
        }else{
            ret=ft5x0x_write_reg(this_client,0x8b,0x00);
             }
        cur_state=state;
        return ret;
    }
}
EXPORT_SYMBOL(TP_charger_change_state);
...
//	msleep(150);
	if(cur_state==1){
                 ft5x0x_write_reg(this_client,0x8b,0x01);
	}
	return 0;
}
			...
			const u8 *data, u32 data_len)
{
	...
	dev_err(&client->dev, "Firmware upgrade successful\n");
...
static int ft5x06_fw_upgrade(struct device *dev, bool force)
{
	...
	dev_err(dev, "Current firmware: %d.%d.%d", data->fw_ver[0],
				data->fw_ver[1], data->fw_ver[2]);
	dev_err(dev, "New firmware: %d.%d.%d", fw_file_maj,
...
static ssize_t ft5x06_fw_version_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
//	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	ssize_t num_read_chars = 0;
	u8 reg_value;
	ft5x0x_read_reg(this_client, 0xa6, &reg_value);
	num_read_chars = sprintf(buf, "TP  fw_version :%x\n", reg_value);
	return num_read_chars;
}

static ssize_t ft5x06_fw_version_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
...
#if 0
 static void ft5x06_firmware_update(struct work_struct *work ){	
	bool update = true;	
	struct ft5x06_ts_data *data =container_of(work, struct ft5x06_ts_data, work);
#if 0 	
	rc = kstrtoul(buf, 10, &val);	
	if (rc != 0)	
		return rc;	
	...
	printk("ft5x06_firmware_update start\n");
	mutex_lock(&data->input_dev->mutex);
	if (!data->loading_fw ) {
		data->loading_fw = true;
		if (ft5x06_fw_upgrade(&data->client->dev, false) < 0 && update) {
	...
	mutex_unlock(&data->input_dev->mutex);
	printk("ft5x06_firmware_update end\n");
} 
#endif
...
#endif
//    INIT_WORK(&data->work, ft5x06_firmware_update);
    err = fts_ctpm_auto_upgrade(client);
	if(err != 0){
	 printk("tp fw auto upgrade error\n");
	}  
	printk("ft5x06 probe success\n"); 
