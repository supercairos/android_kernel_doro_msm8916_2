...
static int RG_RATIO_TYPICAL = 0x139;   // Shoulde changed by Moudle vendor andy
static int BG_RATIO_TYPICAL = 0x11b;  // Shoulde changed by Moudle vendor andy

static struct msm_sensor_ctrl_t *otp_s_ctrl = NULL;

int32_t ov8858_lightarray_write(uint16_t addr, uint16_t data) {
	return otp_s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(otp_s_ctrl->sensor_i2c_client, addr, data,
		MSM_CAMERA_I2C_BYTE_DATA);
}

uint16_t ov8858_lightarray_read(uint16_t addr) {
	uint16_t temp;
...
int ov8858_lightarray_read_otp(struct otp_struct *otp_ptr)
{
	...
	temp1 = ov8858_lightarray_read(0x5002);
	ov8858_lightarray_write(0x5002,(0x00 & 0x08)|(temp1 & (~0x08)));

	// read otp into buffer
	ov8858_lightarray_write(0x3d84, 0xc0); // program disable, manual mode
	ov8858_lightarray_write(0x3d88, 0x70); // start address
	ov8858_lightarray_write(0x3d89, 0x10);
	ov8858_lightarray_write(0x3d8A, 0x72); //end address
	ov8858_lightarray_write(0x3d8B, 0x0a);
	ov8858_lightarray_write(0x3d81, 0x01); // load OTP into buffer
	usleep(10);

	//otp base info and WB Calibration data
	otp_flag = ov8858_lightarray_read(0x7010);
	addr =0;
	...
	if (addr != 0) {
		...
		(*otp_ptr).module_integrator_id = ov8858_lightarray_read(addr);
		(*otp_ptr).lens_id = ov8858_lightarray_read(addr+1);
		(*otp_ptr).production_year = ov8858_lightarray_read(addr+2);
		(*otp_ptr).production_month = ov8858_lightarray_read(addr+3);
		(*otp_ptr).production_day = ov8858_lightarray_read(addr+4);
		temp = ov8858_lightarray_read(addr+7);
		(*otp_ptr).rg_ratio = (ov8858_lightarray_read(addr+5)<<2)+((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (ov8858_lightarray_read(addr+6)<<2)+((temp>>4) & 0x03);
	} else {
	...
	//otp VCM Calibration
	otp_flag = ov8858_lightarray_read(0x7021);
	addr =0;
	if ((otp_flag & 0xc0) == 0x40) {
	...
	if (addr != 0) {
		(*otp_ptr).flag |= 0x20;
		temp = ov8858_lightarray_read(addr+2);
		(*otp_ptr).VCM_start = (ov8858_lightarray_read(addr)<<2) |((temp>>6) & 0x03);
		(*otp_ptr).VCM_end = (ov8858_lightarray_read(addr+1)<<2) |((temp>>4) & 0x03);
	...
	otp_flag = ov8858_lightarray_read(0x7028);
	addr =0;
	if ((otp_flag & 0xc0) == 0x40) {
	...
	if (addr != 0) {
		for(i =0;i<240;i++) {
			(*otp_ptr).lenc[i] = ov8858_lightarray_read(addr+i);
			checksum2 += (*otp_ptr).lenc[i];
		}
		checksum2 = (checksum2)%255+1;
		(*otp_ptr).checksum = ov8858_lightarray_read(addr+240);
	...
	for(i=0x7010;i<=0x720a;i++){
		ov8858_lightarray_write(i,0); //clear otp buffer,recomanded use continuous write to accelarate
	}
	/*
	printk("==========ov8858_lightarray OTP INFO BEGIN==========\n");
	...
	printk("==========ov8858_lightarray OTP INFO END==========\n");
	*/
	//set 0x5002[3] to "1"
	temp1 = ov8858_lightarray_read(0x5002);
	ov8858_lightarray_write(0x5002,(0x08 & 0x08)|(temp1 & (~0x08)));
	return (*otp_ptr).flag;
...
//bit5:0 no otp vcm,1 valid otp vcm
//bit4:0 no otp lenc,1 valid otp lenc
int ov8858_lightarray_apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ( (*otp_ptr).flag & 0x40) {
		...
		if (R_gain > 0x400){
			ov8858_lightarray_write(0x5032,R_gain>>8);
			ov8858_lightarray_write(0x5033,R_gain & 0x00ff);
		}
		if (G_gain > 0x400){
			ov8858_lightarray_write(0x5034,G_gain>>8);
			ov8858_lightarray_write(0x5035,G_gain & 0x00ff);
		}
		if (B_gain > 0x400){
			ov8858_lightarray_write(0x5036,B_gain>>8);
			ov8858_lightarray_write(0x5037,B_gain & 0x00ff);
		}
	}
	//Apply otp lenc Calibration
	if ( (*otp_ptr).flag & 0x10) {
		//printk("dmm debug lenc update come in \n");
		temp = ov8858_lightarray_read(0x5000);
		temp = 0x80 |temp;
		ov8858_lightarray_write(0x5000,temp);
		for(i=0;i<240;i++){
		ov8858_lightarray_write(0x5800+i,(*otp_ptr).lenc[i]);
		}
	}
	return (*otp_ptr).flag;
}

	int ov8858_lightarray_update_otp(struct msm_sensor_ctrl_t *s_ctrl)
	{
	 	struct otp_struct current_otp;
		int temp;
		otp_s_ctrl = s_ctrl;
		temp = ov8858_lightarray_read_otp(&current_otp);
		//printk(" %s read otp info finished flag=0x%x\n",__func__,temp);
		temp = ov8858_lightarray_apply_otp(&current_otp);
